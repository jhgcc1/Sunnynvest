...
def input(request):
	...
	if request.POST.get('calcular'):
		...
		if Comment==None:
			...
			dictStringResult={"spiderS_result":spiderS_result,"simMC_result":simMC_result,"staticMaths_result":staticMaths_result}
			json.dumps(dictStringResult)
			dadosmodels_obj= dadosmodels(entry=entry,yearsTopay=yearsTopay,paymentMethod=paymentMethod,energyconsume=energyconsume, usuario=request.user,radiation1=radiation1,radiation2=radiation2,radiation3=radiation3,pricekwh=pricekwh,
